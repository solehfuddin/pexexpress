...
class OrderRepository(private val apiInterface: ApiInterface) {
    ...
    fun updateReadyToPickup(token: String,image:MultipartBody.Part,jenisUkuran:RequestBody,catatan:RequestBody,diserahkanOleh:RequestBody,statusPengiriman: RequestBody,id:Int) : LiveData<GlobalResponse>{

        val globalResponse = MutableLiveData<GlobalResponse>()
        val errorResponse = MutableLiveData<String>()
        val gson  = Gson()
        apiInterface.updateReadyToPickup(token,image,jenisUkuran,diserahkanOleh,catatan,statusPengiriman,id).enqueue(object :Callback<GlobalResponse>{
            override fun onResponse(
                call: Call<GlobalResponse>,
                response: Response<GlobalResponse>
            ) {
    ...
    fun cancelOrder(token:String,status: String,id:String, @Nullable type:String):LiveData<GlobalResponse>{
        val globalResponse = MutableLiveData<GlobalResponse>()
        val errorResponse = MutableLiveData<String>()
        val gson  = Gson()
        apiInterface.cancelOrder(token,status,id,type).enqueue(object : Callback<GlobalResponse>{
            override fun onResponse(
                call: Call<GlobalResponse>,
                response: Response<GlobalResponse>
            ) {
