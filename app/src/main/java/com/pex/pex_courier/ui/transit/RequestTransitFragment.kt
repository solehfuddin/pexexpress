...
class RequestTransitFragment : Fragment(), CallbackClick {
...
//    private lateinit var recyclerViewAdapter : NewViewAdapter
    ...
    override fun onCreateView(
        ...
        val view=  inflater.inflate(R.layout.fragment_request_transit, container, false)
...
//                recyclerViewAdapter = context?.let { NewViewAdapter(it, activity = activity, this)}!!
        ...
        recyclerViewAdapter.setStatus("26")
    ...
    private fun readData(token:String,limit:Int, offset: Int) {
        provider!!.dataOrderNew(token, 6, limit, offset).observe(viewLifecycleOwner) { res ->
            if (res.success == true) {
